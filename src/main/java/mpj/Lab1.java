...
/*
19
Каждый процесс обменивается сообщениями со всеми остальными
и выводит номера тех процессов у которых ни одно значение не совпадает с его собственными
 */
...
public class Lab1 {
        ...
        for (int i = 0; i < processCount; i++) {
            if (rank != i) {
                ...
                if (intersect(vector, buffer)) {
                    print(format("no commn values in [%s]", i), rank);
                }
            }
        }
    }
    ...
    private static int[] generateArray(int size) {
        return IntStream.range(0, size).map(i -> rnd.nextInt(MAX)).toArray();
    }
